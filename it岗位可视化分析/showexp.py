...
def showexp():
    ...
    city_dic = {'无需经验':[[] for _ in range(9)],'在校生/应届生':[[] for _ in range(9)],'1年经验':[[] for _ in range(9)],'2年经验':[[] for _ in range(9)],'3-4年经验':[[] for _ in range(9)],'5-7年经验':[[] for _ in range(9)],'8-9年经验':[[] for _ in range(9)],'10年以上经验':[[] for _ in range(9)]}
    # 打开清洗后的数据
    ...
    test = []
    for row in result:
        city_dic[row[5]][posi_dic[row[0]]].append(float(row[2]))
    def back():
        root.destroy()
        root.quit()
    # 不同岗位城市可视化函数
