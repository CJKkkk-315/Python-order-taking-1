...
def function1():
    text1.insert("insert","开始扫描时间: %s \n" % time.ctime())
    addr = find_local_ip()
    args = "".join(addr)
    ip_pre = '.'.join(args.split('.')[:-1])
    find_ip(ip_pre)
    text1.insert('insert',"扫描结束时间 %s \n" % time.ctime())
    text1.insert('insert','本次扫描共检测到本网络存在%s台设备\n' % live_ip)
            ...
            while True:
                ...
                try:
                    ...
                    if result_code == 0:
                        ...
                        text2.insert('insert', OPEN_MSG % port)
...
def function2():
    ...
    text2.insert('insert', "[end time] %3ss \n"%(end_time-start_time,))
...
if __name__ == "__main__":
    ...
    e = tk.Entry(window)
    e.place(x=110, y=250)
    bt_login1 = tk.Button(window, text='开始探测', command=lambda :MyThread(function1))
    bt_login1.place(x=300, y=15)
    bt_login2 = tk.Button(window, text='开始扫描', command=lambda :MyThread(function2))
