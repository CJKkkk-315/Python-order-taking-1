...
def main(argv1, argv2, argv3, argv4):
    ...
    # 图像结果保存路径
    classification_image = argv4
    attribute = 'class'
    # 文本结果保存路径
    results_txt = 'result3_GF2.txt'
...
#     plt.subplot(121)
    ...
    cm = confusion_matrix(y,rf.predict(X))#混淆矩阵（真实值，预测值）
     #plt.figure(figsize=(10,7))# 显示大小
     #sn.heatmap(cm, annot=True, fmt='g') #绘制图表函数（annot: 默认为False，为True的话，会在格子上显示数字）
     #plt.xlabel('classes - predicted') #x方向标题
     #plt.ylabel('classes - truth') #y方向标题
     #plt.show()
    ...
    print('精度指数 = {} %'.format(accuracy_score(y_v,X_v)*100), file=open(results_txt, "a"))
     ...
     #plt.xlabel('classes - predicted')#预测值
     #plt.ylabel('classes - truth')#真实值
     #plt.show()
def choosepic1():
    img_RS = e1.get()
...
if __name__ == "__main__":
    ...
    Button(root, width=13, height=2, text='训练样本', command=choosepic1).place(x=30, y=130)
    Button(root, width=13, height=2, text='图像分类', command=choosepic1).place(x=30, y=230)
    Button(root, width=13, height=2, text='精度评价', command=choosepic1).place(x=30, y=330)
    Button(root, width=13, height=2, text='保存图片', command=choosepic1).place(x=30, y=430)
    ...
    # 图像结果保存路径
    classification_image = 'result3_GF2.tif'
