...
with open('用户数据(清洗后)(5000).csv')as f:
    ...
    for row in f_csv:
        users.append(row)
for user in users[4950:]:
    try:
        url = 'https://xueqiu.com/friendships/followers.json?uid=' + str(user[1]) + '&size=50&pageNo='
        ...
        break
with open('用户数据(清洗后)(5000)(粉丝).csv','w',newline='') as f:
    f_csv = csv.writer(f)
    for i in users:
        try:
            f_csv.writerow(i)
